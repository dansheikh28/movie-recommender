...
# Set secret key for sessions (optional but recommended)
...
def recommend():
    ...
    # web scraping to get user reviews from IMDB site with error handling
    ...
    try:
        ...
        reviews_list = ["Reviews unavailable at the moment"]
...
# API endpoints for secure TMDB API calls
...
def get_movie_details():
    try:
        ...
        if response.status_code == 200:
            ...
            return {'error': 'TMDB API request failed'}, response.status_code
...
def get_movie_by_id():
    try:
        ...
        if response.status_code == 200:
            return {'movie_details': response.json()}
        else:
            return {'error': 'TMDB API request failed'}, response.status_code
...
@app.route("/get_cast_details", methods=["POST"])
def get_cast_details():
    try:
        ...
        if response.status_code == 200:
            return {'cast_details': response.json()}
        else:
            return {'error': 'TMDB API request failed'}, response.status_code
...
def get_movie_cast():
    try:
        ...
        if response.status_code == 200:
            return {'cast_data': response.json()}
        else:
            return {'error': 'TMDB API request failed'}, response.status_code
...
@app.route("/get_movie_poster", methods=["POST"])
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data['results'] and len(data['results']) > 0:
                return {'poster_path': data['results'][0]['poster_path']}
            else:
                return {'error': 'No movie found'}, 404
        else:
            return {'error': 'TMDB API request failed'}, response.status_code
